...
#in_out.pyのRANSACがside_infoを使用していた為、roi_infoに変更。またその際current_side_count = len(roi_info)とした一行をコメントアウトした（270行付近）
#アウトコースのスパイク324付近
import os
import time
import math
                        ...
                        if spike_detection_time is None and not stop_flag: # 既に旋回シーケンスに入っていない場合のみ検討
                            ...
                            if is_incourse_candidate or is_outcourse_candidate:
                                ...
                                if course_type == "IN-COURSE":
                                    ...
                                    # アウトコースの場合：「遅延」を開始するためのタイマーをセット
                                    current_side_count = len(side_info) if side_info else 0
                                    is_spike = (current_side_count >= 5)
                                    if is_spike:
                                        print(f"アウトコーススパイクを検出！ {POST_SPIKE_DELAY_S}秒の遅延を開始します。")
                                        spike_detection_time = time.time() # 遅延タイマーをセット
