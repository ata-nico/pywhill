...
STOP_DISTANCE_M   = 1.2         # ★★★ 追加：インコース旋回を開始する距離(m) ★★★
...
SPIKE_DETECT_DELAY_S  = 1.5 # スパイク検出から停止までの遅延時間(秒)

# ▼▼▼【重要】この一行を追加してください ▼▼▼
IN_COURSE_DETECT_RANGE_M = 2.5 # インコースと判断し始める距離(m)
...
spike_detected_timestamp = None # スパイク検出時刻を保持する変数
spike_turn_direction = "NONE"   # スパイク検出時に決定した旋回方向を保持
    ...
    while True:
            ...
            if not stop_flag:
                if First_Dg is None:
                    ...
                    if side_info:
                        ...
                        side_block_history.append(current_side_count) # 移動平均の計算用に履歴は保持
                        ...
                        if direction == 'turn':
                            ...
                            print("out") # ★★★ 出力追加 ★★★
                            # 暫定の旋回方向を計算（スパイク未検出時やインコース用）
                            if side_info:
                                avg_side_theta = np.mean([b[0] for b in side_info])
                                turn_direction = "RIGHT TURN" if avg_side_theta < 0 else "LEFT TURN"
                            ...
                            if front_block_candidate is not None and abs(front_block_candidate['theta']) <= FRONT_TH_DEG:
                                ...
                                if d_front <= IN_COURSE_DETECT_RANGE_M:
                                    ...
                                    if d_front <= STOP_DISTANCE_M:
                                        stop_flag = True

                            if course_type == "OUT-COURSE":
                                is_spike = (current_side_count >= 4)
                                if is_spike:
                                    if spike_detected_timestamp is None:
                                        print("スパイクを検知した") # ★★★ 出力追加 ★★★
                                        # スパイクを初めて検出した瞬間
                                        spike_detected_timestamp = time.time()

                                        # 左右のブロック数をカウント
                                        left_count = sum(1 for b in side_info if b[0] > 0)  # theta > 0 は左
                                        right_count = sum(1 for b in side_info if b[0] < 0) # theta < 0 は右

                                        # ブロック数に基づいて旋回方向を決定・予約
                                        if right_count > left_count:
                                            # 右に多い -> 反時計回り (LEFT TURN)
                                            spike_turn_direction = "LEFT TURN"
                                        elif left_count > right_count:
                                            # 左に多い -> 時計回り (RIGHT TURN)
                                            spike_turn_direction = "RIGHT TURN"
                                        else:
                                            # 同数または0の場合は、従来の平均角度で決定
                                            avg_side_theta = np.mean([b[0] for b in side_info])
                                            spike_turn_direction = "RIGHT TURN" if avg_side_theta < 0 else "LEFT TURN"

                                        print(f"アウトコースの角を検出！ 左:{left_count}, 右:{right_count} -> 「{spike_turn_direction}」を予約")
                                        print(f"タイマー ({SPIKE_DETECT_DELAY_S}秒) を開始します。")

                                    # タイマーが設定されてからの経過時間を確認
                                    elif time.time() - spike_detected_timestamp >= SPIKE_DETECT_DELAY_S:
                                        print(f"遅延時間が経過。予約された旋回({spike_turn_direction})を開始します。")
                                        stop_flag = True

                                else:
                                    # is_spikeがFalseになったらタイマーと予約をリセット
                                    if spike_detected_timestamp is not None:
                                        print("スパイク状態が解消されたため、タイマーと旋回方向をリセットします。")
                                    spike_detected_timestamp = None
                                    spike_turn_direction = "NONE"
                        else:
                            course_type = "STRAIGHT"
                            print("straight") # ★★★ 出力追加 ★★★
        ...
        if len(boxes) == 0:
            ...
            if not side_info:
                ...
                t_all, D_all, _, _, _, _ = min(all_info, key=lambda x: x[1])
                t_side, D_side, _, _, _, _ = min(side_info, key=lambda x: x[1])
                error = D_side - D_all
                status = ""
            ...
            if First_Dg is not None and side_info:
        ...
        if stop_flag:
            if course_type == "IN-COURSE" and front_block_for_turn is not None:
                ...
                if spike_turn_direction != "NONE":
                    turn_direction = spike_turn_direction
                    print(f"スパイク検出時に予約した旋回方向「{turn_direction}」を使用します。")
                else:
                    # この状況は基本発生しないはずだが、念のためフォールバック
                    print(f"警告：予約された旋回方向がありません。現在の平均角度から「{turn_direction}」を決定しました。")

                TIME_PER_DEGREE = 0.142; TURN_SPEED_FOR_CALIBRATION = 5; TARGET_ANGLE = 90.0
                print(f"アウトコース：固定{TARGET_ANGLE}度の旋回({turn_direction})を開始します。"); cv2.waitKey(1500)
                turn_duration = TARGET_ANGLE * TIME_PER_DEGREE
                start_time = time.time()
                while time.time() - start_time < turn_duration:
                    ret, turn_frame = cap.read()
                ...
                while True:
                    if time.time() - search_start_time > 5.0: print("タイムアウト: 新しい壁が見つかりませんでした。"); break
                    ...
                    if cv2.waitKey(1) & 0xFF == ord('q'): break
                    if side_blocks_new:
                        drive_velocity(0, 0); print("新しい壁を発見しました。"); found_wall = True; break
                if found_wall:
                    new_first_dg = min(side_blocks_new, key=lambda b: b['dist'])['dist']
                    if new_first_dg != float('inf'):
                        First_Dg = new_first_dg; print(f"新しい基準距離(First_Dg)を更新しました: {First_Dg:.2f}m")
                ...
                # 旋回が完了したので、タイマーと予約方向をリセット
                spike_detected_timestamp = None
                spike_turn_direction = "NONE"
                print("スパイク検出タイマーと旋回方向をリセットしました。")
            else:
                break
