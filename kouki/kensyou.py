...
try:
    ...
    while True:
                    ...
                    if side_info:
                        ...
                        if direction == 'turn':
                            ...
                            if course_type == "OUT-COURSE":
                                ...
                                # ★★★ stop_flagを立てる代わりに、遅延を開始 ★★★
                                if is_spike and spike_detection_time is None:
                                    print(f"スパイク検出！ {POST_SPIKE_DELAY_S}秒の遅延を開始します。")
        ...
        if stop_flag:
            ...
            if not side_info: drive_velocity(0, -TURN_SPEED_PCT)
            else:
                _, D_all, _, _, _, _ = min(all_info, key=lambda x: x[1]); _, D_side, _, _, _, _ = min(side_info, key=lambda x: x[1])
                error = D_side - D_all
                if error > DIST_THRESHOLD: drive_velocity(0, -TURN_SPEED_PCT); cv2.putText(disp, "Align: Turn Left", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                elif error < -DIST_THRESHOLD: drive_velocity(0, TURN_SPEED_PCT); cv2.putText(disp, "Align: Turn Right", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
                else: drive_velocity(0, 0); aligned = True; cv2.putText(disp, "Aligned", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        # ★★★ スパイク検出後の遅延処理を追加 ★★★
        ...
        elif First_Dg is not None and side_info:
        ...
        if stop_flag:
            if course_type == "IN-COURSE" and front_block_for_turn is not None:
                ...
                TIME_PER_DEGREE = 0.112; TURN_SPEED_FOR_CALIBRATION = 5; TARGET_ANGLE = 90.0
                ...
                while True:
                    if time.time() - search_start_time > 10.0: print("タイムアウト: 新しい壁が見つかりませんでした。"); break
