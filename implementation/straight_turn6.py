...
try:
    while True:
        ...
        if avg_Dg is not None:
            # 閾値を外れたら direction ('turn' or 'straight') に応じてオフセット
            if avg_Dg < lower_thr:
                forward, side = FORWARD_SPEED_PCT, -TURN_SPEED_PCT if direction=='turn' else TURN_SPEED_PCT
            elif avg_Dg > upper_thr:
                forward, side = FORWARD_SPEED_PCT, TURN_SPEED_PCT if direction=='turn' else -TURN_SPEED_PCT
            # 閾値内かつ連続 'turn' 判定が少なければ直進
