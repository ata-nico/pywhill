...
video_path      = os.path.join(OUTPUT_DIR, f"run_{timestamp}.mp4")

# --- ウィンドウ ---
WINDOW_NAME = 'Real-Time RANSAC Detection'
...
if fps == 0: fps = 10.0  # 万が一取得できなければデフォルト10fps
...
while True:
    ...
    # turnなら真正面の点字ブロック表示（黄）
    if direction == 'turn' and roi_info:
        front_block = min(roi_info, key=lambda x: abs(x[0]))  # theta ≒ 0 に最も近い
